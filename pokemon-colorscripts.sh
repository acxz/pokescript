...
OS=$(uname)
test $OS == 'Linux' && PROGRAM=$(readlink -f "$0")
test $OS == 'Darwin' && PROGRAM=$(greadlink -f "$0")
...
_show_random_pokemon(){
    ...
    test $OS == 'Linux' && random_index=$(shuf -i 1-"$NUM_ART" -n 1)
    test $OS == 'Darwin' && random_index=$(gshuf -i 1-"$NUM_ART" -n 1)
